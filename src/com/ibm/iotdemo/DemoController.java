...
import com.ibm.iotdemo.device.ComboSimulationUtility;
...
public class DemoController {
	...
	protected void handleStartButton(ActionEvent event) {
		...
		case "buttonStartDeviceUidPwd":
			selectedDevice = Context.get().selectDevice(0);
			break;
		case "buttonStartDeviceOtp":
			selectedDevice = Context.get().selectDevice(1);
			break;
		case "buttonStartDeviceCert":
			selectedDevice = Context.get().selectDevice(2);
			break;
		}
		...
		if (null != controller) {
			Context.get().setController(controller);
	...
	protected void handleStartButtonAction(ActionEvent event) {
		...
		if (rdoUidPwdDevice.isSelected())
			Context.get().selectDevice(0);
		else if (rdoOtpDevice.isSelected())
			Context.get().selectDevice(1);
		else if (rdoCertDevice.isSelected())
			Context.get().selectDevice(2);

		if (null == Context.get().getSelectedDevice()) {
			startActionStatus.setText("No device selected.");
			return;
		}
		...
		try {
			deviceStarted = Context.get().getSelectedDevice().startDevice();
		} catch (DeviceException exc) {
			System.out.println("Cannot start device: " + exc);
		}
		...
		switch (Context.get().getSelectedDevice().getAuthenticationMethodSupported()) {
		case UIDPWD:
	...
	protected void handleAuthButtonAction(ActionEvent event) {
		...
		DeviceLocalInterface selectedDevice = Context.get().getSelectedDevice();
			...
			case OTP:
				while (true) {
					try {
						...
						if (fieldOtp.getText() != null && !fieldOtp.getText().equals("")) { // if user type STOP in
							// terminal
				...
				// deviceAuthenticated = selectedDevice.login(fieldOtp.getText());
				break;
		...
		/****
		 * // Simulate device delay try { Thread.sleep(2000); }
		 * catch(InterruptedException exc) { // Ignore the interruption }
		 ****/
	...
	protected class SimulationCallback implements SimulationCallbackInterface {
		...
		public void onDataSent(Object... params) {
			...
			} else if (params[0] instanceof ComboSimulationUtility) {
				dataSent.add(new DataSent() {
					{
						timestamp = new SimpleStringProperty(currentTimestamp);
						dataSent = new SimpleStringProperty(params[1].toString());
						sendStatus = new SimpleStringProperty(((boolean) params[2]) ? "Sent" : "Not sent");
					}
				});
			}
		}
